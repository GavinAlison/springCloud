...
# 根据具体profile指定skywalking路径、IP和日志目录
if [[ "${PROFILE}" = "prod" ]]; then
  SKYWALKING_PATH=/data/agent/skywalking-agent.jar
  SKY_IP=39.0.158.101:11800
  LOG_DIR=/data/log/${APP_NAME}
else
  SKYWALKING_PATH=/home/jrxany/skywalking-agent/skywalking-agent.jar
  SKY_IP=10.0.2.102:11800,10.0.9.31:11800
  LOG_DIR=${PROJECT_HOME}/logs
fi
...
if [[ "${REMOTE_DEBUG}" = "true" ]]; then
    JAVA_OPT="${JAVA_OPT} -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5066"
fi
...
if [[ "${SKYWALKING_ENABLE}" = "true" ]]; then
    javaagent="-javaagent:${SKYWALKING_PATH}"
    javaagent="${javaagent} -Dskywalking.agent.service_name=${SKYAGENT_NAME} -Dskywalking.collector.backend_service=${SKY_IP}"
    JAVA_OPT="${JAVA_OPT} ${javaagent}"
fi
