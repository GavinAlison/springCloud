package com.alison.client.base;
...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    // 不接受新任务，等待阻塞队列里的任务处理完成
    private static final int SHUTDOWN = 0 << COUNT_BITS;
    // 不在接收新任务，也不会处理阻塞队列里的任务，并尝试中断正在处理的任务
    private static final int STOP = 1 << COUNT_BITS;
    // 所有任务都结束了，workerCount 是 0
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (; ; ) {
            int c = ctl.get();
            int rs = runStateOf(c);
            // 这个for循环中首先判断线程数是否超过了某个值，如果超过，返回false，不再新建线程。
            // Check if queue empty only if necessary.
            if (rs >= SHUTDOWN &&
                    !(rs == SHUTDOWN &&  // 适应 shutdown方法之后的空闲线程
                            firstTask == null &&
                            !workQueue.isEmpty()))
                return false;

            for (; ; ) {
                //然后CAS改变线程数量记录，如果成功，跳出循环，进行线程的新建。如果不成功，则重试，
                // 并且如果线程池状态发生了变化，还需要继续外层循环，重新进行状态检测。
    ...
    public void execute(Runnable command) {
         ...
         */
        // 这个是先做第一个判断当前线程是不是大于等于核心线程池（说明满了），
        // 如果大于会继续执行, 第二步把提交过来的任务添加到任务队列中去
        int c = ctl.get();
